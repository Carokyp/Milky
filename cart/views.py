...
def _parse_quantity(raw_quantity, default=1):
    """Parse a quantity value, falling back to default on invalid input."""
    try:
...
def _is_ajax(request):
    """Return True if this request was made via the site's JS (fetch)."""
...
def _cart_totals_json(request, cart):
    """Calculate cart totals, delivery cost and free-delivery progress."""
    total = Decimal("0.00")
...
def add_to_cart(request, product_id):
    ...
    try:
        ...
        if existing >= 99:
            messages.info(
                request,
                f"{product.name} is already at the maximum quantity (99).",
            )
        elif new_total < existing + quantity:
            messages.warning(
                request,
                f"Requested quantity was reduced so {product.name} is "
                "capped at 99 in your cart.",
            )
        else:
            messages.success(
                request,
                format_html(
                    "<strong>{}</strong> has been added to your cart!",
                    product.name,
                ),
                extra_tags="cart",
            )
...
def update_cart(request, product_id):
    ...
    try:
        ...
        messages.success(
            request,
            format_html(
                "<strong>{}</strong> has been updated in your cart!",
                product.name,
            ),
...
def remove_from_cart(request, product_id):
    ...
    try:
        ...
        messages.success(
            request,
            format_html(
                "<strong>{}</strong> has been removed from your cart!",
                product.name,
            ),
...
def remove_gift(request):
    """Remove the current gift, freeing up Gift a Can again."""
    try:
