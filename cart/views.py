...
def _cart_totals_json(cart, product_id=None, quantity=None):
    ...
    if product_id is not None:
        ...
        item_qty = int(quantity) if quantity is not None else int(cart.get(str(product_id), 0))
...
def add_to_cart(request, product_id):
    ...
    try:
        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            return JsonResponse(_cart_totals_json(cart, product_id=product_id), status=200)
